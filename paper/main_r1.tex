...
\textcolor{blue}{\paragraph{Roth (2022) HonestDiD.} Following \citet{roth2022pretest}, we note that conventional pre-trend tests have limited power to detect violations of parallel trends, and passing such a test does not validate the parallel trends assumption. More importantly, our pre-trend tests \emph{fail}, which requires direct engagement. \citet{roth2022pretest}'s HonestDiD framework provides bias-corrected confidence intervals under the assumption that the pre-trend continues into the post-period. We have not yet implemented this correction, which represents an important limitation of the current analysis. If the bias-corrected coefficient shrinks substantially or the corrected confidence interval includes zero, the quantitative claim of 35\% displacement would require downward revision. We flag this as a priority for the next iteration of this work.}

\textcolor{blue}{\paragraph{Sun \& Abraham (2021).} With 43 upgrade events occurring at different dates, the standard TWFE estimator may produce biased estimates if treatment effects are heterogeneous across cohorts \citep{dechaisemartin2020twoway, goodmanbacon2021}. Our pre-analysis plan specified implementing the \citet{sunAbraham2021} interaction-weighted estimator as a robustness check. We acknowledge that this estimator has not yet been implemented, which represents a deviation from the pre-analysis plan. Given the staggered timing of upgrade events and the heterogeneity visible in the event-by-event plots, this robustness check is important and will be included in the next revision.}

\textcolor{blue}{\paragraph{Permutation inference.} With only 43 upgrade events---and only 8 with full event windows in the event study figure---standard asymptotic inference may be unreliable. Permutation inference, in which the 43 upgrade dates are randomly reassigned within each firm and the coefficient re-estimated over 1,000+ draws, would provide a distribution-free $p$-value that does not rely on large-sample asymptotics. We have not yet conducted this analysis but acknowledge it as a necessary robustness check. Similarly, wild cluster bootstrap $p$-values would address potential over-rejection in the clustered standard errors with a modest number of events.}

\textcolor{blue}{\paragraph{Leave-one-event-out.} Given the small number of upgrade events, the $-0.43$ coefficient may be driven by one or two large events (e.g., a major Claude or GPT upgrade). A leave-one-event-out analysis---re-estimating the coefficient after dropping each upgrade event in turn---would identify whether the result is robust or driven by a handful of influential observations. We flag this as a priority diagnostic for the next revision.}
...
\textcolor{blue}{We also considered potential bias from staggered treatment timing \citep{dechaisemartin2020twoway, goodmanbacon2021}. For the aggregate same-firm entry coefficient, which is approximately zero, the TWFE weighting issue is less consequential. However, for the family-upgrade specification---where the coefficient is $-0.43$ and treatment effects are likely heterogeneous across cohorts---the TWFE concern is substantive. The \citet{sunAbraham2021} interaction-weighted estimator, which was specified in our pre-analysis plan, has not yet been implemented and represents a priority for the next revision.}
...
\textcolor{blue}{The LLM API market offers a setting where the classic questions of industrial organization---how does entry affect incumbents? does innovation displace or coexist?---can be studied at daily frequency. Our analysis of 385 models over 93 days yields a clear descriptive finding: demand reallocation operates through quality-ladder cannibalization within product families, not through cross-firm displacement. The quantitative estimates carry important caveats---pre-trend concerns, simultaneity bias, small event counts---but the qualitative pattern is robust across specifications.}

\textcolor{blue}{Three priorities for future research follow directly from the limitations of this analysis. First, implementing the \citet{roth2022pretest} HonestDiD correction, \citet{sunAbraham2021} robust estimator, and permutation inference would sharpen the causal interpretation of the family-upgrade coefficient. Second, longer panels would reveal whether the cross-firm null persists as market growth slows or whether displacement eventually emerges as it did in computing \citep{bresnahan1999technological}. Third, data from direct API providers would test whether our results generalize beyond the low-switching-cost segment.}
